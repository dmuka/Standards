...
public class DeleteTests : BaseTestFixture
{
    ...
    private IRequestHandler<Delete.Query<Person>, int> _handler;
    private IValidator<Delete.Query<Person>> _validator;
    ...
    public void Setup()
    {
        ...
        _repository.Setup(_ => _.DeleteAsync(_person, _cancellationToken));
        _repository.Setup(_ => _.SaveChangesAsync(_cancellationToken)).Returns(Task.FromResult(1));

        _cacheService = new Mock<ICacheService>();

        _handler = new Delete.QueryHandler<Person>(_repository.Object, _cacheService.Object);
        _validator = new Delete.QueryValidator<Person>(_repository.Object);
    }
    ...
    {
        // Arrange
        var query = new Delete.Query<Person>(IdInDb);
