...
public class DeleteTests : BaseTestFixture
{
    ...
    private IRequestHandler<Delete.Command<ServiceJournalItem>, int> _handler;
    private IValidator<Delete.Command<ServiceJournalItem>> _validator;
    ...
    public void Setup()
    {
        ...
        _repository.Setup(repository => repository.GetByIdAsync<ServiceJournalItem>(IdInDb, _cancellationToken))
            .Returns(Task.FromResult<ServiceJournalItem?>(_serviceJournalItem));
        _repository.Setup(repository => repository.DeleteAsync(_serviceJournalItem, _cancellationToken));
        _repository.Setup(repository => repository.SaveChangesAsync(_cancellationToken)).Returns(Task.FromResult(1));

        _cacheService = new Mock<ICacheService>();

        _handler = new Delete.CommandHandler<ServiceJournalItem>(_repository.Object, _cacheService.Object);
        _validator = new Delete.CommandValidator<ServiceJournalItem>(_repository.Object);
    }
    ...
    {
        // Arrange
        var command = new Delete.Command<ServiceJournalItem>(IdInDb);
    ...
    {
        // Arrange
        var command = new Delete.Command<ServiceJournalItem>(id);

        // Act
        var result = _validator.TestValidateAsync(command, cancellationToken: _cancellationToken).Result;

        // Assert
        result.ShouldHaveValidationErrorFor(serviceJournalItem => serviceJournalItem.Id);
    }
}