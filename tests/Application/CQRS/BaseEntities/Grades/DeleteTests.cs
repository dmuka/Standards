...
public class DeleteTests : BaseTestFixture
{
    ...
    private IRequestHandler<Delete.Command<Grade>, int> _handler;
    private IValidator<Delete.Command<Grade>> _validator;
    ...
    public void Setup()
    {
        ...
        _repository.Setup(repository => repository.GetByIdAsync<Grade>(IdInDb, _cancellationToken))
            .Returns(Task.FromResult<Grade?>(_grade));
        _repository.Setup(repository => repository.DeleteAsync(_grade, _cancellationToken));
        _repository.Setup(repository => repository.SaveChangesAsync(_cancellationToken)).Returns(Task.FromResult(1));

        _cacheService = new Mock<ICacheService>();

        _handler = new Delete.CommandHandler<Grade>(_repository.Object, _cacheService.Object);
        _validator = new Delete.CommandValidator<Grade>(_repository.Object);
    }
    ...
    {
        // Arrange
        var command = new Delete.Command<Grade>(IdInDb);
